...
/**
 * Search combobox: one query row above one body, where the body holds either
 * the filter bar or the suggestions list.
 *
      ...
      <div
        className={cn(
          ...
          showOverlay
            ...
            : "shadow-[0_1px_2px_rgba(13,21,32,0.04),0_10px_28px_-18px_rgba(13,21,32,0.10)]",
        )}
      >
        <label
          ref={rowRef}
        ...
        >
          ...
          >
            <ul
              id={LISTBOX_ID}
              role="listbox"
              className="flex max-h-[min(60vh,26rem)] flex-col overflow-y-auto p-2"
            >
              {shown === "suggestions" &&
                suggestions.map((s, i) => (
                  <SearchRow
              ...
              {shown === "empty" && (
                <>
                  ...
                  {recommended.length > 0 && (
                    <li role="presentation" className="flex flex-col">
                      {/* Extra room above: this heading follows the last
                          "Недавние" row, and without it the two groups read as
                          one run of items. */}
