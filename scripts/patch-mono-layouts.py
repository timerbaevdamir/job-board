#!/usr/bin/env python3
"""Patch 4 mono scenes in stage import from test import file."""
...
def add_align_left(props: list) -> list:
    out = []
    inserted = False
    for p in props:
        if p.get("key") == "align":
...
def prepare_layouts(layouts: list) -> list:
    result = []
...
def main() -> None:
    ...
    print(f"Updated {updated} scenes -> {STAGE}")
